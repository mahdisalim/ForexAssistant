...
class ForexLiveScraper(BaseScraper):
    """Scraper for ForexLive.com"""
    
    def __init__(self):
        super().__init__("ForexLive", "https://www.forexlive.com")
        self.news_urls = {
            "news": "/news",
    ...
    async def scrape_news(self, currency_pairs: List[str] = None) -> List[NewsArticle]:
        ...
        logger.info(f"Scraped {len(articles)} articles from ForexLive")
        return articles
    ...
    async def _parse_news_list(self, soup: BeautifulSoup, currency_pairs: List[str], category: str) -> List[NewsArticle]:
        ...
        article_elements = soup.select("article, div.article-item, div.post-item")
        
        if not article_elements:
            article_elements = soup.select("a.article-link, div.news-item")
        
        for element in article_elements[:15]:
            try:
                # Extract title
                title_elem = element.select_one("h2, h3, h4, a.title, span.headline")
                if not title_elem:
                    continue
                ...
                # Extract summary
                summary_elem = element.select_one("p, div.excerpt, span.summary, div.description")
                ...
                logger.error(f"Error parsing ForexLive article: {e}")
                continue
    ...
    def extract_article_content(self, soup: BeautifulSoup) -> str:
        ...
        content_elem = soup.select_one("div.article-content, article.content, div.post-content")
